...
public class ItemServiceImpl implements ItemService {
    ...
    public ItemDto createItem(ItemDtoPost itemDtoPost, Long ownerId) {
        ...
        if (itemDtoPost.getRequestId() != null) {
            ItemRequest request = itemRequestRepository.findById(itemDtoPost.getRequestId())
                                                       .orElseThrow(() ->
                                                               new NotFoundException("Запрос с id=" +
                                                                       itemDtoPost.getRequestId() + " не существует"));
    ...
    public List<ItemInfoDto> getAllItemsByOwner(Long ownerId) {
        ...
        Map<Long, List<Booking>> bookingsByItem = ownerBookings
                .stream()
                // терминальный оператор, собирает элементы потока в Map, группируя их по признаку
