...
public interface ItemMapper {
    ...
    @Mapping(target = "owner", ignore = true)  // заполняется в сервисе
    @Mapping(target = "request", ignore = true) // заполняется в сервисе
    Item toItemFromPost(ItemDtoPost itemDtoPost);

    // Item -> ItemInfoDto с бронированиями
    @Mapping(target = "id", source = "item.id") // явно берём id из сущности
    ItemInfoDto toItemInfoDto(Item item, BookingDtoShort lastBooking, BookingDtoShort nextBooking);

    // Postman требует для просмотра вещи пользователем
